---
title: AssetDatabase in ContentGenie
sidebar_label: AssetDatabase
---

# AssetDatabase in ContentGenie

The `AssetDatabase` in ContentGenie is a powerful tool that allows you to manage both local and remote assets. This guide will provide you with examples of how to use the `AssetDatabase`.
...
Automatic gameplay assets also carry a metadata record containing the source URL,
creator, commercial license, attribution, checksum, game-policy evidence, analysis
score, and an `auto_eligible` flag. Only eligible assets are returned by:

```python
AssetDatabase.get_licensed_footage_assets()
```

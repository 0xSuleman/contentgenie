...
def speedUpAudio(tempAudioPath, outputFile, expected_duration=None, max_duration=57):
    tempAudioPath, duration = get_asset_duration(tempAudioPath, False)
    audio_filters = []
    if not expected_duration:
        max_duration = max(20, min(float(max_duration or 57), 59))
        if duration > max_duration:
            audio_filters.append(f'atempo={min(duration / max_duration, 2.0):.5f}')
    else:
        audio_filters.append(f'atempo={max(0.5, min(duration / expected_duration, 2.0)):.5f}')

...
class AbstractContentEngine(ABC):
    ...
    def makeContent(self):
        while (not self.isShortDone()):
            ...
            timings = dict(self._db_stage_timings or {})
            timings[str(currentStep)] = {
                "name": self.stepDict[currentStep].__name__,
                "seconds": elapsed,
            }
            self._db_stage_timings = timings
            self.logger(f"Completed {self.stepDict[currentStep].__name__} in {elapsed:.1f}s")
    ...
    def initializeFFMPEG(self):
        ...
        if not ffmpeg_path:
            raise Exception("FFmpeg, which ContentGenie uses for automated editing, was not found. Install FFmpeg and restart ContentGenie.")

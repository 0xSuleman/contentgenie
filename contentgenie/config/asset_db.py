...
from contentgenie.audio.audio_utils import downloadYoutubeAudio, get_asset_duration
from contentgenie.config.performance import get_bool_setting, get_ffmpeg_binary
from contentgenie.database.db_document import TinyMongoDocument
...
class AssetDatabase:
    ...
    @classmethod
    def add_local_asset(cls, name: str, asset_type: AssetType, path: str, metadata: dict | None = None):
        record = {
        ...
        if source is None or source == 'local':
            for key, asset in cls.local_assets._get().items():
                data.append({'name': key,
                             'type': asset['type'],
                             'link': asset['path'],
                             'source': (asset.get('metadata') or {}).get('source', 'local'),
                             'license': (asset.get('metadata') or {}).get('license_name', ''),
                             'ts': asset.get('ts')
                             })
    ...
    @classmethod
    def get_asset_record(cls, key: str) -> dict | None:
        if key in cls.local_assets._get():
            return cls.local_assets._get(key)
        if key in cls.remote_assets._get():
            return cls.remote_assets._get(key)
        return None

    @classmethod
    def get_licensed_footage_assets(cls) -> list[dict]:
        cls.sync_local_assets()
        assets = []
        for name, asset in cls.local_assets._get().items():
            metadata = asset.get("metadata") or {}
            path = asset.get("path")
            if (
                metadata.get("licensed_footage")
                and metadata.get("auto_eligible")
                and path
                and Path(path).exists()
            ):
                assets.append({"name": name, **asset})
        return sorted(assets, key=lambda item: item.get("ts", ""), reverse=True)

    @classmethod
    def get_licensed_music_assets(cls) -> list[dict]:
        """Return only downloaded tracks with verified, reusable-license evidence."""
        cls.sync_local_assets()
        assets = []
        for name, asset in cls.local_assets._get().items():
            metadata = asset.get("metadata") or {}
            path = asset.get("path")
            if (
                asset.get("type") == AssetType.BACKGROUND_MUSIC.value
                and metadata.get("licensed_music")
                and metadata.get("auto_eligible")
                and path
                and Path(path).exists()
            ):
    ...
    def _get_local_asset_duration(cls, key: str) -> str:
        ...
        asset['ts'] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        cls.local_assets._save({key: asset})
        if 'duration' not in asset or asset.get('duration') is None:
    ...
    def _get_remote_asset_duration(cls, key: str) -> str:
        ...
        asset['ts'] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        cls.remote_assets._save({key: asset})
        cached_video_path = cls._get_cached_youtube_video_path(key, asset)
        if cached_video_path:
            if asset.get('duration') is not None:
                return asset['duration']
            _, duration = get_asset_duration(cached_video_path)
    ...
    @classmethod
    def _is_video_asset(cls, asset: dict) -> bool:
        return 'video' in asset.get('type', '')

    @classmethod
    def _get_video_cache_dir(cls) -> Path:
        cache_dir = Path(os.getenv("YOUTUBE_VIDEO_CACHE_DIR", ".editing_assets/source_cache"))
        cache_dir.mkdir(parents=True, exist_ok=True)
        return cache_dir

    @classmethod
    def _safe_cache_stem(cls, key: str, url: str) -> str:
        safe_key = re.sub(r"[^a-zA-Z0-9_-]+", "_", key).strip("_") or "youtube_asset"
        url_hash = hashlib.sha1(url.encode("utf-8")).hexdigest()[:10]
        return f"{safe_key}_{url_hash}"

    @classmethod
    def _valid_cached_video(cls, path: Path) -> bool:
        return path.exists() and path.is_file() and path.suffix.lower() in VIDEO_EXTENSIONS and path.stat().st_size > 64 * 1024

    @classmethod
    def _get_cached_youtube_video_path(cls, key: str, asset: dict) -> str | None:
        if not cls._is_video_asset(asset):
            return None

        cached_path = asset.get("cached_path")
        if cached_path and cls._valid_cached_video(Path(cached_path)):
            return cached_path

        cache_dir = cls._get_video_cache_dir()
        cache_stem = cls._safe_cache_stem(key, asset["url"])
        for path in sorted(cache_dir.glob(f"{cache_stem}.*"), key=lambda item: item.stat().st_mtime, reverse=True):
            if cls._valid_cached_video(path):
                asset["cached_path"] = str(path)
                cls.remote_assets._save({key: asset})
                return str(path)
        return None

    @classmethod
    def _download_youtube_video_asset(cls, key: str, asset: dict) -> tuple[str | None, float | None]:
        if not get_bool_setting("CACHE_YOUTUBE_BACKGROUNDS", True) or not cls._is_video_asset(asset):
            return None, None

        cached_path = cls._get_cached_youtube_video_path(key, asset)
        if cached_path:
            return cached_path, asset.get("duration")

        cache_dir = cls._get_video_cache_dir()
        cache_stem = cls._safe_cache_stem(key, asset["url"])
        output_template = str(cache_dir / f"{cache_stem}.%(ext)s")
        ...
        ydl_opts = {
            ...
            "noplaylist": True,
            "format": "bestvideo[ext=mp4][height<=1080]/bestvideo[height<=1080]/best[height<=1080]",
        ...
        cached_path = cls._get_cached_youtube_video_path(key, asset)
        if not cached_path:
            raise Exception(f"Failed caching YouTube video asset '{key}'")

        duration = info.get("duration") or asset.get("duration")
        if duration is None:
            _, duration = get_asset_duration(cached_path)
    ...
    def _get_youtube_asset_link(cls, key: str, asset: dict) -> str:
        ...
        cached_video_path, _ = cls._download_youtube_video_asset(key, asset)
        if cached_video_path:
